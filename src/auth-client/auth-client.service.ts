...
export class AuthClientService {
  ...
  async signin(dto: SigninDto) {
    const admin = await this.clientService.findClientByEmail(dto.email);
  ...
  async updateRefreshTokens(id: number, refreshToken: string) {
    ...
    const admin = await this.clientService.updateRefreshToken(
      id,
      refreshTokenHash,
    );
  ...
  async getTokens(userId: number, email: string) {
    const [accessToken, refreshToken] = await Promise.all([
      this.jwt.sign(
        ...
        {
          secret: this.config.get('JWT_SECRET_CLIENT'),
          expiresIn: '24h',
        },
      ),
      this.jwt.signAsync(
        { sub: userId, email: email },
        {
          secret: this.config.get('JWT_REFRESH_SECRET_CLIENT'),
          expiresIn: '1w',
        },
      ),
    ]);
